              ...
              <div className="text-right shrink-0">
                {isCartEditable ? (
                  <div className={`flex items-center gap-1 justify-end mb-1 rounded-full px-1 py-0.5 transition-all ${shakingProd === i.product ? 'animate-shake bg-red-50 ring-2 ring-red-200' : ''}`}>
                    <button onClick={() => onAdjustCartItem(i.product, -1)} className={`bbp-focus-ring w-6 h-6 rounded-full border font-black leading-none transition-colors ${shakingProd === i.product ? 'border-red-300 bg-red-50 text-red-600 hover:bg-red-100' : 'border-pink-200 bg-white text-[#D6006E] hover:bg-pink-50'}`}>
                      -
                    </button>
        ...
        <div className="flex flex-col gap-2 mt-6">
          ...
          {!settings.paymentsOpen ? (
            isReviewStageOpen ? (
              ...
              <button onClick={onSubmitOrder} disabled={isBtnLoading} className={`bbp-focus-ring ${originalBtn} w-full py-4`}>
                {isBtnLoading ? "Saving..." : "Save Order"}
              </button>
            )
          ) : (
            <button onClick={onOpenPayModal} disabled={cartList.length === 0} className="bbp-focus-ring w-full bg-[#008040] text-white font-bold py-4 rounded-full uppercase tracking-widest text-sm shadow-md hover:scale-[0.98] transition-transform active:scale-95 disabled:opacity-50">Pay Now</button>
          )}
        </div>
      </div>
    </aside>
